...
async function main() {
  ...
  const houjicha = await g("Houjicha Powder", "gm", 36);
  const fm = await g("Fresh Milk", "ml", 0);
  const om = await g("Oat Milk", "ml", 0);
  const cup = await g("Paper Cup 8oz", "pcs", 50);

  let r = await prisma.recipe.findFirst({ where: { cafeId, name: "Houjicha (Hot)" } });
  if (r) { console.log("Already exists"); return; }

  r = await prisma.recipe.create({ data: { name: "Houjicha (Hot)", description: "Whisked houjicha with steamed milk", cafeId } });
  console.log("+ Recipe: Houjicha (Hot)");
  ...
  await prisma.variationIngredient.createMany({ data: [
    { variationId: fv.id, ingredientId: houjicha.id, quantityPerServing: 5, subtotalOverrideInCents: 179 },
  ...
  await prisma.variationStep.createMany({ data: [
    { variationId: fv.id, stepNumber: 1, instruction: "In the whisking cup, add 5g Houjicha Powder" },
    { variationId: fv.id, stepNumber: 2, instruction: "Add 50ml of tap water" },
    { variationId: fv.id, stepNumber: 3, instruction: "Whisk the tap water and Houjicha powder together until smooth" },
  ...
  console.log("+ Fresh Milk variation (RM 2.59)");

  // Oat Milk - RM 2.99
  const ov = await prisma.recipeVariation.create({ data: { recipeId: r.id, name: "Oat Milk" } });
  await prisma.variationIngredient.createMany({ data: [
    { variationId: ov.id, ingredientId: houjicha.id, quantityPerServing: 5, subtotalOverrideInCents: 179 },
  ...
  await prisma.variationStep.createMany({ data: [
    { variationId: ov.id, stepNumber: 1, instruction: "In the whisking cup, add 5g Houjicha Powder" },
    { variationId: ov.id, stepNumber: 2, instruction: "Add 50ml of tap water" },
    { variationId: ov.id, stepNumber: 3, instruction: "Whisk the tap water and Houjicha powder together until smooth" },
  ...
  console.log("+ Oat Milk variation (RM 2.99)");
  console.log("Done!");
}
