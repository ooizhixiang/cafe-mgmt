...
async function main() {
  ...
  let r = await prisma.recipe.findFirst({ where: { cafeId, name: "Cappuccino (Cold)" } });
  if (r) { console.log("Already exists"); return; }

  r = await prisma.recipe.create({ data: { name: "Cappuccino (Cold)", description: "Iced cappuccino with fresh or oat milk", cafeId } });
  console.log("+ Recipe: Cappuccino (Cold)");
  ...
  await prisma.recipeStep.createMany({ data: [
    ...
    { recipeId: r.id, stepNumber: 2, instruction: "Add 200ml fresh/oat milk into the cup" },
    { recipeId: r.id, stepNumber: 3, instruction: "Add half cup of ice" },
  ]});

  // Fresh Milk variation - RM 2.50
