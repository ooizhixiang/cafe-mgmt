...
async function main() {
  ...
  const matcha = await g("Tenzo Matcha Powder", "gm", 38);
  const fm = await g("Fresh Milk", "ml", 0);
  const om = await g("Oat Milk", "ml", 0);
  const cup = await g("Paper Cup 8oz", "pcs", 50);

  let r = await prisma.recipe.findFirst({ where: { cafeId, name: "Matcha Latte (Hot)" } });
  if (r) { console.log("Already exists"); return; }

  r = await prisma.recipe.create({ data: { name: "Matcha Latte (Hot)", description: "Whisked matcha with steamed milk", cafeId } });
  console.log("+ Recipe: Matcha Latte (Hot)");
  ...
  await prisma.variationIngredient.createMany({ data: [
    { variationId: fv.id, ingredientId: matcha.id, quantityPerServing: 4, subtotalOverrideInCents: 150 },
  ...
  await prisma.variationStep.createMany({ data: [
    { variationId: fv.id, stepNumber: 1, instruction: "In the whisking cup, add 4g Tenzo Matcha Powder" },
    { variationId: fv.id, stepNumber: 2, instruction: "Add 50ml of tap water" },
    { variationId: fv.id, stepNumber: 3, instruction: "Whisk the tap water and matcha powder together until smooth" },
  ...
  await prisma.variationIngredient.createMany({ data: [
    { variationId: ov.id, ingredientId: matcha.id, quantityPerServing: 4, subtotalOverrideInCents: 150 },
  ...
  await prisma.variationStep.createMany({ data: [
    { variationId: ov.id, stepNumber: 1, instruction: "In the whisking cup, add 4g Tenzo Matcha Powder" },
    { variationId: ov.id, stepNumber: 2, instruction: "Add 50ml of tap water" },
    { variationId: ov.id, stepNumber: 3, instruction: "Whisk the tap water and matcha powder together until smooth" },
